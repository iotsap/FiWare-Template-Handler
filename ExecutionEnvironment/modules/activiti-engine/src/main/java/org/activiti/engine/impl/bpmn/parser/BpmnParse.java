...
import noNamespace.NotifyConditionType.Enum;
...
import org.activiti.engine.impl.bpmn.behavior.SensingTaskActivityBehavior;
...
public class BpmnParse extends Parse {
  ...
  /** A map for storing the physical entities */
  protected Map<String, PhysicalEntity> physicalEntities = new HashMap<String, PhysicalEntity>();

  /** A map for storing the physical associations */
  protected Map<String, PhysicalAssociation> physicalAssociations = new HashMap<String, PhysicalAssociation>();

  /** A map for storing the data associations */
  protected Map<String, AbstractDataAssociation> dataAssociations = new HashMap<String, AbstractDataAssociation>();
  ...
  private XmlOptions xmlBeansValidationOptions;

  @SuppressWarnings("rawtypes")
  private ArrayList xmlBeansValidationErrors;
  ...
  protected void parseRootElement() {
    ...
    parseCollaboration();
    ensurePhysicalAssociationsForIotTasks();
  ...
  public void parseProcessDefinitions() {
    ...
    boolean parseOnlyExecutableProcesses = processEngineConfiguration.isParseOnlyExecutableProcesses();

    for (Element processElement : rootElement.elements("process")) {
      boolean processProcess = true;
      if (parseOnlyExecutableProcesses) {
  ...
  private EntityId extractEntityId(Element participant) {
    Element entityIdElem = participant.element("entityId");
    EntityId entityId = EntityId.Factory.newInstance();
    entityId.setId(entityIdElem.attribute("id"));
    entityId.setIsPattern(Boolean.parseBoolean(entityIdElem.attribute("isPattern")));
    entityId.setType(entityIdElem.attribute("type"));
    return entityId;
  }

  public void parseMessageFlows(Element collaborationElement) {
    for (Element messageFlowElement : collaborationElement.elements("messageFlow")) {
      String sourceRef = messageFlowElement.attribute("sourceRef");
      PhysicalEntity peAsSource;
      if (null == sourceRef || "".equals(sourceRef)) {
        peAsSource = null;
      } else {
        peAsSource = physicalEntities.get(sourceRef);
      }
      String targetRef = messageFlowElement.attribute("targetRef");
      PhysicalEntity peAsTarget;
      if (null == targetRef || "".equals(targetRef)) {
        peAsTarget = null;
      } else {
        peAsTarget = physicalEntities.get(targetRef);
      }
      
      if (null == peAsSource) {
        if (null == peAsTarget) {
          // both are null, so this is a normal message flow -> do nothing at the moment
        } else {
          // target is physical entity -> actuation
          // actuation is not possible with NGSI tooling
          addError("messageFlow from '" + sourceRef + "' to physical entity '" + peAsTarget.getName() + "' is not possible with NGSI tooling", messageFlowElement);
        }
      } else {
        if (null == peAsTarget) {
          // source is physical entity -> sensing task or NGSI start event
          parseMessageFlowtoPE(messageFlowElement, targetRef, peAsSource);
        } else {
          // both are physical entities, this is nonsense
          addError("messageFlow between two physical entities '" + peAsSource.getName() + "' and '" + peAsTarget.getName() + "' is not allowed", messageFlowElement);
        }
      }
    }
  }

  protected void parseMessageFlowtoPE(Element messageFlowElement, String targetRef, PhysicalEntity peAsSource) {
    ActivityImpl targetActivity = findActivityById(targetRef);
    if (null == targetActivity) {
      addError("no activity found for association of physical entity '" + peAsSource.getName() + "'", messageFlowElement);
    } else {
      ActivityBehavior targetActivityBehavior = targetActivity.getActivityBehavior();
      boolean targetIsSensingTask = targetActivityBehavior instanceof SensingTaskActivityBehavior;
      //WHY can a start *event* be an activity? However, also events are listed as ActivityImpl
      boolean targetIsNGSIStartEvent = targetActivity.getProperty("type").equals("ngsiStartEvent");
      boolean targetIsValid = targetIsNGSIStartEvent || targetIsSensingTask;
      if (targetIsValid) {
        storePhysicalAssociation(messageFlowElement, peAsSource, targetActivity);
      } else {
        addError("activity '" + targetActivity.getProperty("name") + "' for association on physical entity '" + peAsSource.getName() + "' is not a sensingTask or a NGSI start event", messageFlowElement);
      }
    }
  }

  protected ActivityImpl findActivityById(String activityId) {
    if (!elementIds.contains(activityId)) {
      return null;
    }
    for (ProcessDefinitionEntity processDef: processDefinitions) {
      for (ActivityImpl activityImpl : processDef.getActivities()) {
  ...
  protected void storePhysicalAssociation(Element messageFlowElement, PhysicalEntity physicalEntity, ActivityImpl activity) {
    String id = messageFlowElement.attribute("id");
    PhysicalAssociation physicalAssociation = new PhysicalAssociation();
    physicalAssociation.setPhysicalEntity(physicalEntity);
    physicalAssociation.setActivity(activity);
    physicalAssociations.put(id, physicalAssociation);
    activity.getPhysicalAssociations().add(physicalAssociation);
  }

  protected void ensurePhysicalAssociationsForIotTasks() {
    for (ProcessDefinitionEntity pde : processDefinitions) {
      for (ActivityImpl activityImpl : pde.getActivities()) {
        if (activityImpl.getActivityBehavior() instanceof SensingTaskActivityBehavior) {
          if (activityImpl.getPhysicalAssociations().size() < 1) {
            addError("IotTask '" + activityImpl.getProperty("name") + "' is not connected to any Physical Entity", null);
          }
        }
      }
    }
  ...
  protected void parseDataObject(Element processElement, ScopeImpl parentScope) {
	  for (Element dataObjectElement : processElement.elements("dataObject")) {

		  String id = dataObjectElement.attribute("id");
		  String isCollection = dataObjectElement.attribute("isCollection");
		  String name = dataObjectElement.attribute("name");
		  
		  if (LOGGER.isLoggable(Level.FINE)) {
			  LOGGER.fine("Parsing data object " + id);
		  }
		  
		  DataObject dataObject = parentScope.createDataObject(id);
...
protected void parsePostponedElements(Element scopeElement, ScopeImpl parentScope, HashMap<String, Element> postponedElements) {
  ...
  protected void parseAssociations(Element scopeElement, ScopeImpl parentScope) {
    for (Element associationElement : scopeElement.elements("association")) {
      ...
      if(sourceActivity == null && !elementIds.contains(sourceRef)) {
        ...
        if(null != sourceActivity && sourceActivity.getProperty("type").equals("compensationBoundaryCatch")) {
  ...
  protected void parseProcessDefinitionStartEvent(ActivityImpl startEventActivity, Element startEventElement, Element parentElement, ScopeImpl scope) {
    ...
    Element ngsiEventDefinition = startEventElement.element("ngsiEventDefinition");
    ...
    } else if(ngsiEventDefinition != null) {
      parseNgsiStartEventDefinition(ngsiEventDefinition, startEventActivity, processDefinition, scope, parentElement);
    }
  }
  
  
  private void parseNgsiStartEventDefinition(Element ngsiEventDefinition, ActivityImpl startEventActivity, ProcessDefinitionEntity processDefinition, ScopeImpl scope, Element parentElement) {
    //the connection to the physical entity is made from the physical entity side. therefore, the type property is needed later
    startEventActivity.setProperty("type", "ngsiStartEvent");
    NGSIStartEventActivityBehaviour startEventActivityBehaviour = new NGSIStartEventActivityBehaviour();
    DataObjectDataOutputAssociation dataAssociation = findTargetDataObjectForNGSIEvent(ngsiEventDefinition, startEventActivity, scope, parentElement);
    startEventActivityBehaviour.setDataAssociation(dataAssociation);
    parseOptionalNGSIEventBehaviourWithErrorHandling(ngsiEventDefinition, startEventActivityBehaviour);
    startEventActivity.setActivityBehavior(startEventActivityBehaviour);
  }

  private void parseOptionalNGSIEventBehaviourWithErrorHandling(Element ngsiEventDefinition, NGSIStartEventActivityBehaviour startEventActivityBehaviour) {
    prepareXMLBeansErrorHandling();
    try {
      parseOptionalNGSIEventBehaviour(ngsiEventDefinition, startEventActivityBehaviour);
    } catch (DatatypeConfigurationException e) {
      addError(e.getMessage(), ngsiEventDefinition);
    }
    @SuppressWarnings("rawtypes")
    Iterator iter = xmlBeansValidationErrors.iterator();
    while (iter.hasNext())
    {
      addError(iter.next().toString(), ngsiEventDefinition);
    }
  }

  private void parseOptionalNGSIEventBehaviour(Element ngsiEventDefinition, NGSIStartEventActivityBehaviour startEventActivityBehaviour) throws DatatypeConfigurationException {
    AttributeList attributeList = parseAttributeList(ngsiEventDefinition);
    startEventActivityBehaviour.setAttributeList(attributeList);
    List<NotifyCondition> notifyConditions = parseNotifyConditions(ngsiEventDefinition);
    startEventActivityBehaviour.setNotifyConditions(notifyConditions);
    Duration duration = parseDuration(ngsiEventDefinition);
    startEventActivityBehaviour.setDuration(duration);
  }

  private Duration parseDuration(Element ngsiEventDefinition) throws DatatypeConfigurationException {
    Element durationElement = ngsiEventDefinition.element("duration");
    if(durationElement == null)
      return null;
    Duration duration = DatatypeFactory.newInstance().newDuration(durationElement.getText());
    return duration;
  }

  private List<NotifyCondition> parseNotifyConditions(Element ngsiEventDefinition) {
    List<Element> notifyConditionElements = ngsiEventDefinition.elements("notifyCondition");
    if(notifyConditionElements == null) {
      return null;
    }
    List<NotifyCondition> notifyConditions = new LinkedList<NotifyCondition>();
    for(Element notifyConditionElement : notifyConditionElements) {
      notifyConditions.add(parseNotifyCondition(notifyConditionElement));
    }
    return notifyConditions;
  }

  private NotifyCondition parseNotifyCondition(Element notifyConditionElement) {
    NotifyCondition notifyCondition = NotifyCondition.Factory.newInstance();
    Enum type = parseNotifyConditionType(notifyConditionElement);
    notifyCondition.setType(type);
    Element restrictionElement = notifyConditionElement.element("restriction");
    if(restrictionElement != null) {
      notifyCondition.setRestriction(restrictionElement.getText());
    }
    Element condValueListElement = notifyConditionElement.element("condValueList");
    if(condValueListElement != null) {
      notifyCondition.setCondValueList(parseCondValueList(condValueListElement));
    }
    return notifyCondition;
  }

  private Enum parseNotifyConditionType(Element notifyConditionElement) {
    Element typeElement = notifyConditionElement.element("type");
    if(typeElement == null) {
      addError("There must be a notify condition type", notifyConditionElement); 
    }
    Enum type = NotifyConditionType.Enum.forString(typeElement.getText());
    if(type == null) {
      addError("The condition type is not valid", notifyConditionElement); 
    }
    return type;
  }

  private CondValueList parseCondValueList(Element condValueListElement) {
    CondValueList condValueList = CondValueList.Factory.newInstance();
    List<Element> condValueElements = condValueListElement.elements("condValue");
    for(Element condValueElement : condValueElements) {
      condValueList.addCondValue(condValueElement.getText());
    }
    return condValueList;
  }

  private AttributeList parseAttributeList(Element ngsiEventDefinition) {
    Element attributeListElement = ngsiEventDefinition.element("attributeList");
    if(attributeListElement == null)
      return null;
    AttributeList attributeList = AttributeList.Factory.newInstance();
    List<Element> attributeElements = attributeListElement.elements("attribute");
    for(Element attributeElement : attributeElements) {
      attributeList.addAttribute(attributeElement.getText());
    }
    return attributeList;
  }

  @SuppressWarnings("rawtypes")
  private void prepareXMLBeansErrorHandling() {
    xmlBeansValidationErrors = new ArrayList();
    xmlBeansValidationOptions = new XmlOptions();
    xmlBeansValidationOptions.setErrorListener(xmlBeansValidationErrors);
  }

  //not very elegant since the associations are not parsed at this point
  private DataObjectDataOutputAssociation findTargetDataObjectForNGSIEvent(Element ngsiEventDefinition, ActivityImpl startEventActivity, ScopeImpl scope, Element parentElement) {
    for (Element associationElement : parentElement.elements("association")) {
      String sourceRef = associationElement.attribute("sourceRef");
      String targetRef = associationElement.attribute("targetRef");
      if(sourceRef.equals(startEventActivity.getId())) {
        DataObjectDataOutputAssociation outputAssociation = checkTargetDataObjectForNGSIEvent(ngsiEventDefinition, startEventActivity, scope, associationElement, targetRef);
        if(outputAssociation != null) {
          return outputAssociation;
        }
      }
    }
    addError("The NGSI start event has no data object as sink", ngsiEventDefinition);
    return null;
  }

  private DataObjectDataOutputAssociation checkTargetDataObjectForNGSIEvent(Element ngsiEventDefinition, ActivityImpl startEventActivity, ScopeImpl scope, Element associationElement, String targetRef) {
    DataObject dataObject = scope.findDataObject(targetRef);
    if(dataObject == null) {
      addError("A NGSI start event must have an outgoing association only to a data object", ngsiEventDefinition);
    }
    DataObjectDataOutputAssociation outputAssociation = new DataObjectDataOutputAssociation(startEventActivity, dataObject);
    addDataAssociationToList(associationElement, outputAssociation);
    dataObject.getDataAssociations().add(outputAssociation);
    return outputAssociation;
  }
  ...
  protected void parseActivity(Element activityElement, Element parentElement, ScopeImpl scopeElement, HashMap<String, Element> postponedElements) {
    ...
    } else if (activityElement.getTagName().equals("sensingTask")) {
      activity = parseSensingTask(activityElement, scopeElement);
  ...
  public ActivityImpl parseScriptTask(Element scriptTaskElement, ScopeImpl scope) {
    ...
    parseActivityDataElements(scriptTaskElement, scope, activity);
  ...
  protected AbstractDataAssociation parseDataOutputAssociation(Element dataAssociationElement, ScopeImpl scope) {
	  String targetRef = dataAssociationElement.element("targetRef").getText();
	  
	  if (dataAssociationElement.element("sourceRef") != null) {
		  String sourceRef = dataAssociationElement.element("sourceRef").getText();
		  DataObject dataObject = scope.findDataObject(targetRef);
		  ActivityImpl activity = scope.findActivity(sourceRef);
		  
		  if (dataObject != null && activity != null) {
			  DataObjectDataOutputAssociation outputAssociation = new DataObjectDataOutputAssociation(activity, dataObject);
			  addDataAssociationToList(dataAssociationElement, outputAssociation);
			  dataObject.getDataAssociations().add(outputAssociation);
			  return outputAssociation;
		  }
	  }
  ...
  protected AbstractDataAssociation parseDataInputAssociation(Element dataAssociationElement, ScopeImpl scope) {
	  String targetRef = dataAssociationElement.element("targetRef").getText();
	  
	  if (dataAssociationElement.element("sourceRef") != null) {
		  String sourceRef = dataAssociationElement.element("sourceRef").getText();
		  DataObject dataObject = scope.findDataObject(sourceRef);
		  ActivityImpl activity = scope.findActivity(targetRef);
		  
		  if (dataObject != null && activity != null) {
			  DataObjectDataInputAssociation inputAssociation = new DataObjectDataInputAssociation(dataObject, activity);
			  addDataAssociationToList(dataAssociationElement, inputAssociation);
			  dataObject.getDataAssociations().add(inputAssociation);
  ...
  protected AbstractDataAssociation parseDataOutputAssociation(Element dataAssociationElement) {
    ...
    if (dataAssociationElement.element("sourceRef") != null) {
      String sourceRef = dataAssociationElement.element("sourceRef").getText();
      MessageImplicitDataOutputAssociation messageImplicitDataOutputAssociation = new MessageImplicitDataOutputAssociation(targetRef, sourceRef);
      addDataAssociationToList(dataAssociationElement, messageImplicitDataOutputAssociation);
      return messageImplicitDataOutputAssociation;
    } else {
      ...
      addDataAssociationToList(dataAssociationElement, dataOutputAssociation);
      return dataOutputAssociation;
    }
  }

  private void addDataAssociationToList(Element element, AbstractDataAssociation association) {
    String id = element.attribute("id");
    if (null == id) {
      return;
    }
    if (dataAssociations.containsKey(id)) {
      return;
    }
    dataAssociations.put(id, association);
  }
  ...
  /**
   * Parses a sensing task.
   * TODO improve parsing when schema evolves
   */
  public ActivityImpl parseSensingTask(Element sensingTaskElement, ScopeImpl scope) {
    String contextAttribute = sensingTaskElement.attribute("contextAttribute");
    ActivityImpl activity = createActivityOnScope(sensingTaskElement, scope);
    SensingTaskActivityBehavior activityBehaviour = new SensingTaskActivityBehavior();
    activityBehaviour.setContextAttribute(contextAttribute);
    activity.setActivityBehavior(activityBehaviour);
    
    parseActivityDataElements(sensingTaskElement, scope, activity);
    
    parseExecutionListenersOnScope(sensingTaskElement, activity);

    for (BpmnParseListener parseListener : parseListeners) {
      parseListener.parseSensingTask(sensingTaskElement, scope, activity);
    }
    
    return activity;
  }
  
  protected void parseActivityDataElements(Element taskElement, ScopeImpl scope, ActivityImpl activity) {
	  TaskActivityBehavior activityBehaviour = (TaskActivityBehavior) activity.getActivityBehavior();
	  
	  Element ioSpecificationElement = taskElement.element("ioSpecification");
	  if (ioSpecificationElement != null) {
		  IOSpecification ioSpecification = this.parseIOSpecification(ioSpecificationElement);
		  activityBehaviour.setIoSpecification(ioSpecification);
	  }

	  for (Element dataAssociationElement : taskElement.elements("dataInputAssociation")) {
		  AbstractDataAssociation dataAssociation = this.parseDataInputAssociation(dataAssociationElement, scope);
		  if (dataAssociation != null) {
			  activityBehaviour.addDataInputAssociation(dataAssociation);
		  }
	  }

	  for (Element dataAssociationElement : taskElement.elements("dataOutputAssociation")) {
		  AbstractDataAssociation dataAssociation = this.parseDataOutputAssociation(dataAssociationElement, scope);
		  if (dataAssociation != null) {
			  activityBehaviour.addDataOutputAssociation(dataAssociation);
		  }
	  }
  }
 ...
 ActivityImpl activity = createActivityOnScope(transactionElement, scope);
  ...
  public void parseBPMNShape(Element bpmnShapeElement) {
    ...
    if (bpmnElement != null && !"".equals(bpmnElement)) {
      ...
      if (null != physicalEntities.get(bpmnElement)) {
        PhysicalEntity pe = physicalEntities.get(bpmnElement);
        parseDIBounds(bpmnShapeElement, pe);
        return;
      }
      ...
      for (ProcessDefinitionEntity processDefinition : getProcessDefinitions()) {
        ...
        if (activity != null) {
          ...
          if(lane != null) {
            ...
            DataObject dataObject = processDefinition.findDataObject(bpmnElement);
            if (null != dataObject) {
              // The shape represents a data object
              parseDIBounds(bpmnShapeElement, dataObject);
  ...
  protected void parseWaypoints(Element bpmnEdgeElement, HasWaypoints target) {
