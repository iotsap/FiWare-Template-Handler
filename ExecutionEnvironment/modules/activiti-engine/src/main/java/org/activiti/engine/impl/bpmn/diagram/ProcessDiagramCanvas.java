...
public class ProcessDiagramCanvas {
  ...
  protected static Image ACTUATIONTASK_IMAGE;
  protected static Image SENSINGTASK_IMAGE;
  protected static Image TIMER_IMAGE;
  protected static Image ERROR_THROW_IMAGE;
  protected static Image ERROR_CATCH_IMAGE;
  protected static Image PHYSICAL_ENTITY_IMAGE;
  protected static Image NGSI_IMAGE;
  ...
  static {
    try {
      ...
      ACTUATIONTASK_IMAGE = ImageIO.read(ReflectUtil.getResourceAsStream("org/activiti/engine/impl/bpmn/deployer/actuation.png"));
      SENSINGTASK_IMAGE = ImageIO.read(ReflectUtil.getResourceAsStream("org/activiti/engine/impl/bpmn/deployer/sensing.png"));
      ...
      PHYSICAL_ENTITY_IMAGE = ImageIO.read(ReflectUtil.getResourceAsStream("org/activiti/engine/impl/bpmn/deployer/physicalEntity.png"));
      NGSI_IMAGE = ImageIO.read(ReflectUtil.getResourceAsStream("org/activiti/engine/impl/bpmn/deployer/ngsi.png"));
    } catch (IOException e) {
  ...
  protected List<PhysicalEntity> drawnPhysicalEntitys = new ArrayList<PhysicalEntity>();
  ...
  public void drawNgsiStartEvent(int x, int y, int width, int height) {
    drawStartEvent(x, y, width, height, NGSI_IMAGE);
  }
  ...
  public void drawActuationTask(String name, int x, int y, int width, int height) {
    drawTask(name, x, y, width, height);
    g.drawImage(ACTUATIONTASK_IMAGE, x + 7, y + 7, ICON_SIZE, ICON_SIZE, null);
  }

  public void drawSensingTask(String name, int x, int y, int width, int height) {
    drawTask(name, x, y, width, height);
    g.drawImage(SENSINGTASK_IMAGE, x + 7, y + 7, ICON_SIZE, ICON_SIZE, null);
  }
  ...
  public void drawPhysicalEntity(PhysicalEntity physicalEntity) {
    if (drawnPhysicalEntitys.contains(physicalEntity)) {
      return;
    }
    drawnPhysicalEntitys.add(physicalEntity);
    int x = physicalEntity.getX();
    int y = physicalEntity.getY();
    int width = physicalEntity.getWidth();
    int height = physicalEntity.getHeight();
    String name = physicalEntity.getName();
    g.drawRect(x, y, width, height);
    if (name != null) {
      String text = fitTextToWidth(name, width);
      int textX = x + ((width - fontMetrics.stringWidth(text)) / 2);
    ...
    g.drawImage(PHYSICAL_ENTITY_IMAGE, x + 7, y + 7, 52, 42, null);
  }

  public void drawDataObject(DataObject dataObject) {
    int x = dataObject.getX();
    int y = dataObject.getY();
    int width = dataObject.getWidth();
    int height = dataObject.getHeight();
    int triangleSize = 15;
    // draw document stencil
    g.drawLine(x, y, x + width - triangleSize, y);
    g.drawLine(x, y, x, y + height);
    g.drawLine(x, y + height, x + width, y + height);
    g.drawLine(x + width, y + triangleSize, x + width, y + height);
    g.drawLine(x + width - triangleSize, y, x + width, y + triangleSize);
    g.drawLine(x + width - triangleSize, y, x + width - triangleSize, y + triangleSize);
    g.drawLine(x + width, y + triangleSize, x + width - triangleSize, y + triangleSize);
    // add text
  ...
  public void drawDataAssociation(AbstractDataAssociation dataAssociation) {
    List<Integer> waypoints = dataAssociation.getWaypoints();
    for (int i = 2; i < waypoints.size(); i += 2) {
      if (i < waypoints.size() - 2) {
        drawMessageFlowWithoutArrow(waypoints.get(i - 2), waypoints.get(i - 1), waypoints.get(i), waypoints.get(i + 1));
      } else {
        drawDataAssociation(waypoints.get(i - 2), waypoints.get(i - 1), waypoints.get(i), waypoints.get(i + 1));
      }
    }
  }
