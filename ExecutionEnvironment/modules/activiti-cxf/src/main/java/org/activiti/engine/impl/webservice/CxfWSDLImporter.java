...
public class CxfWSDLImporter implements XMLImporter {
  ...
  private void importStructure(Mapping mapping) {
    ...
    for (Entry<String, JFieldVar> entry : fields.entrySet()) {
    	// The JFieldVar's type might be a narrowed type (generic).
    	// Therefore we have to erasure it before trying to load
    	// its class object.
    	JType jType = entry.getValue().type();
    	JType erasuredType = jType.erasure();
		Class<?> fieldClass = ReflectUtil.loadClass(erasuredType.boxify().fullName());
